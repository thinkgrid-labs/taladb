...
- **Schema-evolution normalization — persist-on-read + React Native** — read-time `migrateDocument(doc, fromVersion)` (arbitrary JS, all runtimes) and structural `syncSchema.renames` (import-time, browser + Node) both ship; what remains is (1) an opt-in *persist* mode so a lazily-migrated document is written back to storage — not just transformed on the way out — so filters/indexes on the new shape match stored old documents without an explicit `openDB({ migrations })` rewrite; and (2) carrying the structural `importChangesValidated` plumbing into the React Native JSI binding. *(Read-time migration, structural renames, validate-on-import, and `_v` upgrades are documented in [Schema & Sync Standards](/guide/schema-and-sync-standards); additive-only + origin-authoritative shape remains the safe path.)*
