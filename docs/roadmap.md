...
Subpath exports (`/server`, `/client`) follow the next-auth convention and keep the RSC boundary explicit in the import path. Verified end-to-end (real Chrome client → handlers → TalaDB-backed store). *Still to come: an example app in the repo.*
